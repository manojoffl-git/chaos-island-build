...
func build_island() -> void:
    ...
    var edge_positions: Array[Vector3] = [
        ...
        Vector3(-21, 0.0, 0), Vector3(21, 0.0, 2),
        Vector3(-19, 0.0, 18), Vector3(0, 0.0, 20), Vector3(18, 0.0, 18)
    ]
    for p in edge_positions:
        mesh_box(Vector3(9, 0.9, 9), Color("#5b9b55"), island, p, "CoastChunk")

    var collision: CollisionShape3D = $Island/Collision
...
func build_lake() -> void:
    ...
    for i in range(3):
        ...
        post.rotation_degrees.x = 0.0

func build_hills() -> void:
    var hills: Node3D = $Hills
    var hill_data: Array[Vector3] = [
        Vector3(-13, 1.8, -11), Vector3(16, 2.1, 10), Vector3(-15, 1.4, 12),
        Vector3(5, 1.0, 16), Vector3(-2, 1.2, -16)
    ]
    for i in range(hill_data.size()):
        var p: Vector3 = hill_data[i]
        var hill: MeshInstance3D = mesh_cylinder(4.8, p.y * 2.0, Color("#6cab59"), hills, Vector3(p.x, p.y, p.z), "Hill_%d" % i)
        hill.scale = Vector3(1.25, 1.0, 0.9)
